...
export function RegisterContent() {
  ...
  useEffect(() => {
    ...
    if (roleParam === "parent" || roleParam === "student") {
      setForm(prev => ({ ...prev, role: roleParam as "student" | "parent" }));
    }
  }, [searchParams]);
  ...
  const addCustomBoard = () => {
    if (
      customBoard.trim() &&
      !form.board.toLowerCase().includes(customBoard.toLowerCase())
    ) {
      updateForm("board", customBoard.trim());
      setCustomBoard("");
  ...
  const handleRegister = async () => {
    setLoading(true);
    try {
      console.log("[Register] Sending form with role:", form.role);
      console.log(
        "[Register] Full form object:",
      ...
      console.log("[Register] Response user role from backend:", user.role);

      // Always trust backend's role determination
      console.log("[Register] Using backend-returned role:", user.role);
      ...
      console.log("[Register] Redirecting to:", destination);
          ...
          {step === 2 && form.role === "student" && (
            <div className="space-y-5">
              ...
              <div>
                ...
                {/* Custom Board Option */}
                {!showCustomBoard && !BOARDS.includes(form.board) && (
                  <div className="mb-3 p-3 rounded-xl bg-primary-50 dark:bg-primary-500/10 border border-primary-200 dark:border-primary-500/30">
                    <p className="text-sm font-semibold text-primary-700 dark:text-primary-300">
                      Custom Board:{" "}
                      <span className="font-bold">{form.board}</span>
                      <button
                        type="button"
                ...
                {showCustomBoard && (
                  <div className="flex gap-2 mb-3">
                    ...
                    <button
                      ...
                      className="px-4 py-2 rounded-xl bg-primary-500 hover:bg-primary-600 text-white font-semibold text-sm transition-all"
                    >
                      Add
                    </button>
                    <button
              ...
              <div>
                ...
                {form.subjects.filter(s => !SUBJECTS.includes(s)).length >
                  0 && (
                    ...
                    {form.subjects
                      ...
                      .map(s => (
                        <button
                          ...
                          type="button"
                          onClick={() => toggleSubject(s)}
                          className="px-3 py-1.5 rounded-xl text-sm font-semibold border-2 border-green-500 bg-green-50 dark:bg-green-500/10 text-green-600 dark:text-green-400 transition-all hover:opacity-80"
                        >
                          {s} ✓
                        </button>
                      ))}
